...
class RaquetesUsadas {
	...
	public function pedidoshoje(){
		$date = date("Y-m-d");
		$sql = "select count(*) as total from `" . DB_PREFIX . "order` where order_status_id != 0 and `store_id` = 1 and date(date_added) between date('".$date."') and now()";
	...
	public function pedidosmes(){
		...
		$sql = "select count(*) as total from `" . DB_PREFIX . "order` where order_status_id != 0 and `store_id` = 1 and date(date_added) between date('".$initialDate."')  and now()";
		$resultado = $this->db->query($sql);
		$response = null;
		while($pedido = mysqli_fetch_assoc($resultado)){
			$response = $pedido;
	...
	public function vendashoje(){
		$sql = "select sum(`total`) as total from `" . DB_PREFIX . "order` where order_status_id != 0 and `date_added` between curdate( ) and now( ) and `store_id`= 1";
		$resultado = $this->db->query($sql);
		$response = null;
	...
	public function vendasmes(){
		$sql = "select sum(`total`) as total from `" . DB_PREFIX . "order` where order_status_id !=0 and `date_added` between date_sub( date(now()), interval day( now() ) -1 day ) and now() and `store_id`= 1";
		$resultado = $this->db->query($sql);
		$response = null;
	...
	public function ticketmedio(){
		...
		$sql1 = "select sum(`total`) as total from `" . DB_PREFIX . "order` where order_status_id !=0 and `date_added` between date_sub( date(now()), interval day( now() ) -1 day ) and now() and `store_id`= 1";
		$resultado1 = $this->db->query($sql1);

		$sql2 = "select count(*) as total from `" . DB_PREFIX . "order` where order_status_id != 0 and `store_id` = 1 and date(date_added) between date('".$initialDate."')  and now()";
